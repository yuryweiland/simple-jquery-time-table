(function ($) {
  ...
  DayScheduleSelector.prototype.renderHeader = function () {
    ...
    $.each(days, function (i, _) { html += '<th class="day-label" data-day='+ i +'>' + (stringDays[i] || '') + '</th>'; });
  ...
  DayScheduleSelector.prototype.renderRows = function () {
    ...
    $.each(generateDates(start, end, interval), function (i, d) {
      ...
      $el.append('<tr><td class="time-label" data-time="' + hhmm(d) + '">' + hmmAmPm(d) + '</td>' + daysInARow + '</tr>');
    });
  };
  ...
  DayScheduleSelector.prototype.attachEvents = function () {
	...
	this.$el.on('click', '.day-label', function () {
		var day = $(this).data('day');
		var daySlots = document.querySelectorAll('.time-slot[data-day="' + day + '"]');
	...
	this.$el.on('click', '.time-label', function () {
        ...
        var timeSlots = document.querySelectorAll('.time-slot[data-time="' + time + '"]');
